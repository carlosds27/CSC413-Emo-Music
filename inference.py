...
def predict(types, task, file_path):
    if types not in ["midi_like", "remi", "wav"]:
        return -1, -1
    if task not in ["ar_va", "arousal", "valence"]:
        return -1, -1
    cuda = 'cuda:0'
    device = cuda if cuda and torch.cuda.is_available() else 'cpu'
    if cuda:
        print('GPU name: ', torch.cuda.get_device_name(device=cuda))
    ...
    if types == "midi_like":
        quantize_midi = magenta_extractor(file_path)
        model_input = torch.LongTensor(quantize_midi).unsqueeze(0)
        prediction = model(model_input.to(cuda))
    elif types == "remi":
        quantize_midi = remi_extractor(file_path, event_to_int)
        model_input = torch.LongTensor(quantize_midi).unsqueeze(0)
        prediction = model(model_input.to(cuda))
    elif types == "wav":
        model_input = torch_sox_effect_load(file_path, 22050).mean(0, True)
        ...
        prediction = model(audio_sample.to(cuda))
    ...
    print("========")
    print(file_path, " is emotion", pred_label)
